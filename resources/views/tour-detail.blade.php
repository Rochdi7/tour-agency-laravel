@php
    ...
    $tourImageUrl = $tourImageObject ? asset('storage/' . $tourImageObject->image_path) : asset('assets/img/sahara-merzouga-camel-tour-sunset-morocco.webp'); // Fallback image
            ...
            <div class="row text-center">
                <div class="col-12">
                    <div class="breadcrumb-content">
                        ...
                        @if($tour->subtitle)
                            <p class="breadcrumb-subtitle" id="tourSubtitle"
...
@endif
        ...
        @php 
            $image = $tour->images->first(); // Fetch the first image related to the tour
        @endphp

        @if ($image)
            <img src="{{ asset('storage/' . $image->image_path) }}" {{-- Image path from storage --}}
                 alt="{{ $image->alt ?? $tour->title }}" {{-- Alt text from database or tour title --}}
                 title="{{ $image->caption ?? $tour->title }}" {{-- Caption as title attribute --}}
                                            ...
                                            <div class="accordion-style2 accordion flex-grow-1" id="tourItineraryAccordion">
                                                @foreach ($tour->itineraryDays as $day)
                                                    <div class="accordion-item">
                                                        ...
                                                        <h6 class="accordion-header" id="headingItinDay{{ $day->id }}">
                                                            <button
                                        ...
                                        <div class="accordion-style2 accordion mt-3" id="accordionAccommodations"> {{-- Added mt-3 --}}
                                            @foreach ($tour->accommodations as $index => $accommodation)
                                                <div class="accordion-item">
                                                    ...
                                                    <h6 class="accordion-header" id="headingAccommodation{{ $index }}">
                                                        <button
    ...
    @else
        <p class="mt-3">A map illustrating the route for the {{ $tour->title }} is currently unavailable.</p>
    @endif
</div>
    ...
    <input id="inq_arrival_date" name="arrival_date" type="text"
        ...
        required aria-label="Preferred Arrival Date" 
